...
- **`data_mock.py`**  
  用于模拟真实场景下不同位置的人流密度数据，并定时发送到服务器接口。

- **`http_client.py`**  
  实时监测某一区域的人流密度，并通过 HTTP 接口发送到服务器进行汇总与分析。

- **`kalman_people_flow_density.py`**  
  用卡尔曼滤波来实时估算某个区域的人流密度。

- **`sensor_simulator.py`**  
  用于管理和监控红外传感器和 WiFi 探针，实现人流和设备数量自动采集与记录。

- **`simplified_sensor_simulator.py`**  
  传感器模拟器，用于模拟红外传感器和 WiFi 探针的数据生成和收集过程。

- **`density_sarima_model`**  
  预训练的预测模型

- **`density_predictor.py`**  
  基于 SARIMA 模型的人流密度预测工具，可以训练模型、进行预测、评估性能并绘制预测图表。

- **`server.py`**  
  基于 FastAPI 框架构建的人流密度监测与预测服务的代码实现，提供了一个 RESTful API，用于接收、存储、查询人流密度数据，并能够预测未来的人流密度。

- **`index.html`**  
  人流密度监测系统的网页，提供实时数据展示、热图、位置信息和人流预测功能。

- **`script.js`**  
  用于展示和预测人流密度数据的网页脚本，包含实时数据加载、热图更新、统计信息显示和预测功能。

- **`styles.css`**  
  一个响应式的 CSS 样式表，定义了网页的布局、颜色、按钮、表单、统计信息、地图、模态框等元素的外观和交互效果。
